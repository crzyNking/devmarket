...
function DevMarketLoader() {
  ...
  const steps = [
    ...
    { icon: '🛒', label: 'Loading marketplace' },
    { icon: '🚀', label: 'Launching DevMarket' },
  ];
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(phaseTimer); clearInterval(dotsTimer); };
  }, []);

  return (
    <div className="dm-loader">
      {/* Animated grid background */}
      <div className="dm-loader__grid" aria-hidden="true">
        {Array.from({ length: 64 }).map((_, i) => (
          <div key={i} className="dm-loader__cell" style={{ animationDelay: `${(i * 0.04) % 2}s` }} />
        ))}
      </div>

      {/* Floating code tokens */}
      <div className="dm-loader__tokens" aria-hidden="true">
        {['const', 'import', '=>', '{}', '<>', 'async', 'return', '[]', 'export', 'await'].map((token, i) => (
          <span key={i} className="dm-loader__token" style={{ '--i': i }}>{token}</span>
        ))}
      </div>
      ...
      <div className="dm-loader__card">
        ...
        <div className="dm-loader__logo-wrap">
          ...
          <div className="dm-loader__orbit dm-loader__orbit--2">
            <div className="dm-loader__orbit-dot dm-loader__orbit-dot--2" />
          </div>
          <div className="dm-loader__logo-icon">🚀</div>
        </div>
        ...
        <p className="dm-loader__tagline">Where Developers Trade &amp; Share</p>
        ...
        <div className="dm-loader__steps">
          {steps.map((s, i) => (
            <div key={i} className={`dm-loader__step ${i <= phase ? 'dm-loader__step--active' : ''} ${i < phase ? 'dm-loader__step--done' : ''}`}>
              <span className="dm-loader__step-icon">{i < phase ? '✓' : s.icon}</span>
              <span className="dm-loader__step-label">{s.label}</span>
            </div>
          ))}
        </div>

        {/* Dots */}
        <div className="dm-loader__dots" aria-label="Loading">
          {[0, 1, 2].map(i => (
            <span key={i} className={`dm-loader__dot ${dots > i ? 'dm-loader__dot--on' : ''}`} />
          ))}
        </div>
      </div>
    </div>
...
function App() {
  ...
  async function loadPublicData() {
    try {
      ...
      if (s.data?.length > 0) { hasData = true; dispatch({ type: 'SET_CODE_SNIPPETS', payload: s.data.map(x => ({...x, author: x.author_name, authorAvatar: x.author_avatar, likedBy: x.liked_by || [], date: new Date(x.created_at).toLocaleDateString()})) }); }
...
function Marketplace() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch({ type: 'ADD_NOTIFICATION', payload: { message: `❌ Failed to publish listing: ${error.message}`, type: 'error', time: new Date().toLocaleTimeString(), read: false } });
    }
    setSubmitting(false);
  };
...
function Advertise() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch({ type: 'ADD_NOTIFICATION', payload: { message: `❌ Failed to publish app: ${error.message}`, type: 'error', time: new Date().toLocaleTimeString(), read: false } });
    }
    setSubmitting(false);
  };
...
function CodeSharing() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      dispatch({ type: 'ADD_NOTIFICATION', payload: { message: `❌ Failed to share snippet: ${error.message}`, type: 'error', time: new Date().toLocaleTimeString(), read: false } });
    }
    setSubmitting(false);
  };
  ...
  const handleLike = async (snippet) => {
    ...
    try {
      await supabase.from('code_snippets').update({ likes: newLikes, liked_by: newLikedBy }).eq('id', snippet.id);
    } catch (error) {
